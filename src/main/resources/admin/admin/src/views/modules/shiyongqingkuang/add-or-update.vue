<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="工位名称" prop="gongweimingcheng">
          <el-select v-model="ruleForm.gongweimingcheng" placeholder="请选择工位名称">
            <el-option
                v-for="(item,index) in gongweimingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="在位人数" prop="zaiweirenshu">
          <el-input v-model="ruleForm.zaiweirenshu" 
              placeholder="在位人数" clearable  :readonly="ro.zaiweirenshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="在位人数" prop="zaiweirenshu">
              <el-input v-model="ruleForm.zaiweirenshu" 
                placeholder="在位人数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gongweimingcheng = true;
          ...
          if(o=='zaiweirenshu'){
            this.ruleForm.zaiweirenshu = obj[o];
	    this.ro.zaiweirenshu = true;
            continue;
          }
          if(o=='dengjiriqi'){
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.gongweimingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shiyongqingkuang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shiyongqingkuangCrossAddOrUpdateFlag = false;
                  this.parent.search();
