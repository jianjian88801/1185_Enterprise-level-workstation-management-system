<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="分配工位" prop="fenpeigongwei">
          <el-select v-model="ruleForm.fenpeigongwei" placeholder="请选择分配工位">
            <el-option
                v-for="(item,index) in fenpeigongweiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="分配工位" prop="fenpeigongwei">
	      <el-input v-model="ruleForm.fenpeigongwei"
                placeholder="分配工位" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="分配原因" prop="fenpeiyuanyin">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.fenpeiyuanyin" label="分配原因" prop="fenpeiyuanyin">
                    <span v-html="ruleForm.fenpeiyuanyin"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yuangongxingming = true;
          ...
          if(o=='fenpeigongwei'){
            this.ruleForm.fenpeigongwei = obj[o];
	    this.ro.fenpeigongwei = true;
            continue;
          }
          if(o=='fenpeiyuanyin'){
	...
	this.ruleForm.fenpeiyuanyin = this.ruleForm.fenpeiyuanyin.replace(reg,'../../../springbooti8ts3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `gongweifenpei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gongweifenpeiCrossAddOrUpdateFlag = false;
                  this.parent.search();
